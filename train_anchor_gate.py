...
def get_args_parser():
    ...
    parser.add_argument('--gate_reset', default=False, action=argparse.BooleanOptionalAction)
...
def main(args):
    ...
    for idx in tqdm(range(len(anchor_idx))):
        ...
        lora_path = os.path.join(args.anchor_path, f'user_{user["user_id"]}/lora_ckpt.pt')
        ...
        torch.save(model.gate_state_dict(), os.path.join(user_out_dir, 'gate_ckpt.pt'))
